...
import 'package:chatapp/Classes_Widgets/own_meeting.dart';
import 'package:chatapp/Classes_Widgets/replay-meeting.dart';
import 'package:chatapp/Classes_Widgets/set_assigment.dart';
import 'package:chatapp/Classes_Widgets/view_class.dart';
...
import '../Screens/Camera_Screen_group.dart';
...
class ClassChat extends StatefulWidget {
  ...
  ClassChat(this.source, this.currentGroup, this.members, this.messages,
      this.allContact);
  _ClassChat createState() => _ClassChat();
}

class _ClassChat extends State<ClassChat> {
  XFile? image;
  String? _FileName;
  int indexUser = 0;
                                      ...
                                      builder: (context, snapshot) {
                                        ...
                                        for (var massege in masseges!) {
                                          ...
                                          massegeWidget[index]
                                              .groupuser
                                              .delete = massegeText;
                                          massegeWidget[index].groupuser.seen =
                                              massegetime;
                                          massegeWidget[index].groupuser.time =
                                              massegetype;
                                          if (massegetype == "false") {
                                            UpdateSeenMSG(
                                                massegeWidget[index].id, idddd);
                                          }
                                        }
                                        ...
                                        else if(massegeWidget[index].typemsg=="assigment"){
                                                return AssigmentCard(massegeWidget[index],widget.source,widget.currentGroup);
                                              }
                                        else if (massegeWidget[index].type=="meeting"){
                                          if (massegeWidget[index].type ==
                                              "source") {
                                            return OwnMeeting(massegeWidget[index],widget.source.name);
                                          }
                                          else{
                                            return ReplayMeeting(massegeWidget[index],widget.source.name);
                                          }
                                        }
                                        else {
                                          return Text("");
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  iconCreation(
                      Icons.video_camera_back, "Video", Colors.redAccent),
                  SizedBox(
                    width: 40,
                  ),
              ...
              Row(
                ...
                children: [
                  ...
                  iconCreation(Icons.assignment, "Assigmnment", Colors.green),
                  SizedBox(
                    width: 40,
                  ),
  ...
  void setMassegetaget(List<MessageModel> msg) async {
    try {
      for (int i = 0; i < msg.length; i++) {
        ...
        await for (var snapshot in _firestore2
            .collection('MassegeGroup')
            .doc(msg[i].id)
            .collection('users')
          ...
          final delete = usersMap2!['delete'];
          final seen = usersMap2!['seen'];
          final time = usersMap2!['time'];
  ...
  void UpdateSeenMSG(String id, String id2) async {
    ...
    try {
      final docRef = _firestore2
          .collection("MassegeGroup")
          .doc(id)
          .collection('users')
          .doc(id2);
      final updates = <String, dynamic>{"seen": "true", 'time': tt};
  ...
  void GetAudioDevice(String sourceId) async {
    try {
      ...
      if (_result != null) {
        ...
        final path =
            "classes/${widget.currentGroup.groupid}/chat/audio/${_FileName}";
  ...
  void getFilesDevice(String sourceId) async {
    try {
      ...
      if (_result != null) {
        ...
        filetoDisplay = File(pickfile!.path.toString());
        final path =
            "classes/${widget.currentGroup.groupid}/chat/file/${_FileName}";
        final ref = FirebaseStorage.instance.ref().child(path);
        final uploadTask = ref.putFile(filetoDisplay!);
        final snapshot = await uploadTask!.whenComplete(() {});
        final urlDownload = await snapshot.ref.getDownloadURL();
  ...
  Future getImage(ImageSource media, String sourceId) async {
    ...
    final path =
        "classes/${widget.currentGroup.groupid}/chat/photo/${image!.name}";
  ...
  Future getVedioSend(ImageSource media, String sourceId) async {
    ...
    final path =
        "classes/${widget.currentGroup.groupid}/chat/vedio/${image!.name}";
    final file = File(image!.path);
    final ref = FirebaseStorage.instance.ref().child(path);
    final uploadTask = ref.putFile(file);
    final snapshot = await uploadTask!.whenComplete(() {});
  ...
  Future<void> sendNotification(
    ...
    String serverKey =
        'AAAA8zqSgR0:APA91bHjvWusiXeUAcHfL_61mXoBwiYGOuev5b1f4KrHsR3L03ssODwyWTdooABf1O9M4pKtUdvYUpaqIF3uX-Ut2HAfS2ITholY5EGorFAam3Iomx4r3X3y60QJrneaCfEQnkaRefkP';
  ...
  void setUsers(String id, String em) async {
    try {
      ...
      final doc = ref
          ...
          .collection('users')
          .doc(ids)
          .set({'email': em, 'seen': 'false', 'delete': 'false', 'time': ''});
    } catch (e) {
      print(e);
    }
  }
